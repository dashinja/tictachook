...
export default function Game() {
  ...
  function handleSquareClick(index) {
    ...
    if (!board[index] && !getWinner()) {
      ...
      setCurrentPlayer(
        currentPlayer === PLAYER_2_SYMBOL ? PLAYER_1_SYMBOL : PLAYER_2_SYMBOL,
      )
    }
  }
  ...
  return (
    <div>
      ...
      <h1>
        Status: {getWinner() ? `Game Over! Refresh to Play Again!` : ""}
      </h1>
      <h2>
        Your Turn: {getWinner() ? "" : `Player ${currentPlayer}`}
      </h2>
      <h2>
        Winner: {getWinner() ? `Player ${getWinner()}!` : `N/A`}
      </h2>
